...
public class LobbyingBillsDao extends MyJdbcDaoSupport implements LobbyingBillsInterface {
	...
	@Override
	public List<TopLobbyiedBills> getTop() throws SQLException {
		String sql = "SELECT lob_bills.B_ID, lob_bills.Bill_Name, lobbyissue.issueID, lobbyissue.issue, Lobbying.uniqid, Lobbying.amount\n" 
				+ "FROM lob_bills\n" 
				+ "LEFT JOIN lobbyissue\n" 
				+ "ON lob_bills.si_id = lobbyissue.SI_ID\n"
				+ "LEFT JOIN Lobbying\n" 
				+ "ON lobbyissue.uniqID = Lobbying.uniqid\n"
				+ "ORDER BY Lobbying.amount DESC";
		Connection connection = null;
		PreparedStatement selectStmt = null;
		...
		try {
			...
			List<TopLobbyiedBills> topPaidBill = new ArrayList<TopLobbyiedBills>();
			while (results.next()) {
				topPaidBill.add(
						new TopLobbyiedBills(results.getString("B_ID"), results.getString("Bill_Name"), results.getString("issueID"), results.getString("issue"), results.getInt("amount")));
			}
			return topPaidBill;
		} catch (SQLException e) {
			e.printStackTrace();
